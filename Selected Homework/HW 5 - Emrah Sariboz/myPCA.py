...
class pca_reduction:
    ...
    def apply_PCA(self, X_training, variance):
        pca = PCA(variance)
        pca.fit(X_training)       
        return pca.transform(X_training)
    
    def plot_optimal_num(self):
        plt.plot(np.cumsum(self.classifierName.explained_variance_ratio_))
        plt.title('Finding optimal number of componnent')
